...
- [1. ansible 部署FATE集群手册](#1-ansible-部署fate集群手册)
        - [1.0.1. 概述](#101-概述)
            - [1.0.1.1. FATE简介](#1011-fate简介)
        - [1.0.2. 部署手册](#102-部署手册)
            - [1.0.2.1. 环境依赖](#1021-环境依赖)
            - [1.0.2.2. 依赖条件和前置操作](#1022-依赖条件和前置操作)
            - [1.0.2.3. 组件信息](#1023-组件信息)
            - [1.0.2.4. 基本概念与原理](#1024-基本概念与原理)
                - [1.0.2.4.1. 版本和部署包形态](#10241-版本和部署包形态)
                - [1.0.2.4.2. 部署流程](#10242-部署流程)
                - [1.0.2.4.3. 部署形态](#10243-部署形态)
                - [1.0.2.4.4. 部署角色（fate）](#10244-部署角色fate)
                - [1.0.2.4.5. 部署模块（fate）](#10245-部署模块fate)
                - [1.0.2.4.6. 部署支持polling(fate)](#10246-部署支持pollingfate)
                - [1.0.2.4.7. 路由支持(fate)](#10247-路由支持fate)
                    - [1.0.2.4.7.1. 配置路由的参数](#102471-配置路由的参数)
                    - [1.0.2.4.7.2. **2.4.7.2 路由逻辑**](#102472-2472-路由逻辑)
                    - [1.0.2.4.7.3. 路由地址](#102473-路由地址)
                - [1.0.2.4.8. 证书支持(fate)](#10248-证书支持fate)
                - [1.0.2.4.9. 支持配置Spark参数（fate）](#10249-支持配置spark参数fate)
            - [1.0.2.5. 辅助脚本和配置文件](#1025-辅助脚本和配置文件)
                - [1.0.2.5.1. 下载脚本和下载配置文件](#10251-下载脚本和下载配置文件)
                    - [1.0.2.5.1.1. 下载脚本的使用](#102511-下载脚本的使用)
                    - [1.0.2.5.1.2. 下载配置文件](#102512-下载配置文件)
                - [1.0.2.5.2. 部署辅助脚本和部署配置文件](#10252-部署辅助脚本和部署配置文件)
                    - [1.0.2.5.2.1. 部署辅助脚本使用指引](#102521-部署辅助脚本使用指引)
                    - [1.0.2.5.2.2. 使用部署辅助脚本进行初始化](#102522-使用部署辅助脚本进行初始化)
                    - [1.0.2.5.2.3. 使用部署辅助脚本生成证书](#102523-使用部署辅助脚本生成证书)
                    - [1.0.2.5.2.4. 使用部署辅助脚本进行部署或卸载](#102524-使用部署辅助脚本进行部署或卸载)
                    - [1.0.2.5.2.5. 配置文件场景示例](#102525-配置文件场景示例)
                - [1.0.2.5.3. ansible配置文件](#10253-ansible配置文件)
                    - [1.0.2.5.3.1. 配置base信息](#102531-配置base信息)
                    - [1.0.2.5.3.2. 配置fate基础信息](#102532-配置fate基础信息)
                    - [1.0.2.5.3.3. 配置Exchange信息](#102533-配置exchange信息)
                    - [1.0.2.5.3.4. 配置Host信息](#102534-配置host信息)
                    - [1.0.2.5.3.5. 配置Guest信息](#102535-配置guest信息)
                    - [1.0.2.5.3.6. 配置任务列表](#102536-配置任务列表)
                    - [1.0.2.5.3.7. 配置主机列表](#102537-配置主机列表)
            - [1.0.2.6. 部署流程](#1026-部署流程)
                - [1.0.2.6.1. 下载](#10261-下载)
                    - [1.0.2.6.1.1. 下载离线包](#102611-下载离线包)
                    - [1.0.2.6.1.2. 下载非离线包](#102612-下载非离线包)
                    - [1.0.2.6.1.3. 按需下载模块的资源包](#102613-按需下载模块的资源包)
                    - [1.0.2.6.1.4. 按需自编译模块的资源包](#102614-按需自编译模块的资源包)
                - [1.0.2.6.2. 使用初始化脚本生成部署配置文件](#10262-使用初始化脚本生成部署配置文件)
                - [1.0.2.6.3. 调整参数并生成ansible配置文件](#10263-调整参数并生成ansible配置文件)
                - [1.0.2.6.4. 执行ping测试(可选)](#10264-执行ping测试可选)
                - [1.0.2.6.5. 生成证书(可选)](#10265-生成证书可选)
                - [1.0.2.6.6. 执行部署（按需）](#10266-执行部署按需)
                - [1.0.2.6.7. 检查服务](#10267-检查服务)
                - [1.0.2.6.8. 执行测试(fate)](#10268-执行测试fate)
            - [1.0.2.7. 服务验证](#1027-服务验证)
                - [1.0.2.7.1. 服务进程验证](#10271-服务进程验证)
                - [1.0.2.7.2. 单边测试](#10272-单边测试)
                - [1.0.2.7.3. 双边测试](#10273-双边测试)
            - [1.0.2.8. 服务运维](#1028-服务运维)
            - [1.0.2.9. 打包](#1029-打包)
                - [1.0.2.9.1. 下载在线包](#10291-下载在线包)
                - [1.0.2.9.2. 执行脚本进行打包](#10292-执行脚本进行打包)
        - [1.0.3. 特定操作指引](#103-特定操作指引)
            - [1.0.3.1. 使用自签证书](#1031-使用自签证书)
            - [1.0.3.2. 单边部署使用证书场景](#1032-单边部署使用证书场景)
            - [1.0.3.3. mysql使用外部数据库](#1033-mysql使用外部数据库)
...
本文将介绍使用ansible部署脚本进行FATE集群的部署和升级组件。
...
​       部署一个或者多个模块：mysql、eggroll、fate_flow和fateboard
...
- 组件配置逻辑
  ...
  - 非exchange情景，必须设置default

  - nodemanger可以设置多个ip

  - exchange的rollsite可以设置多个ip
...
###### 1.0.2.4.7.2. **2.4.7.2 路由逻辑**

- 部署三方，host和guest的默认路由是exchange。 exchange没有默认路由。
- 部署两方（没exchange），互为默认路由。
...
- 初始化下载配置文件
  ...
  参数说明：
          ...
          version： 资源包大版本号
          minversion： 资源包小版本号
  使用示例：	bash build/build.sh init fate 1.7.0 release
...
​		host_special_routes：host端额外路由，一般指向exchange，数组格式：party_id:ip:port，exchange例：host_special_routes: [ 'default:192.168.0.88:9370' ]
...
##### 1.0.2.5.3. ansible配置文件

###### 1.0.2.5.3.1. 配置base信息
...
guest:
  ...
  fate_flow:
    ...
    dbname: "fate_flow"	---数据库名称
    proxy: rollsite			---可选值：rollsite|fateflow|nginx，fateflow和nginx用于spark
    use_deserialize_safe_module: false		---默认false
  fateboard:
    enable: True		---true为需要部署此模块，False则否
    ips:
    - 192.168.1.1		---只支持部署一台主机
